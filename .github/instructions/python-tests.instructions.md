...
- Tests should be readable and easy to understand for a human. So do not use `\t` or `\n` because they are not easy to read for humans. If fixtures are not provided in the prompt, prefer to use real examples to build them. If unsure, check parallel tests or fetch the actual data from `public/data`.

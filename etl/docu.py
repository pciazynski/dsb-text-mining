...
import datetime
import time
from settings import datadir, webdir
import hashlib
def main(argv=None):
    ...
    for file in os.listdir(webdir + "php"):
        with open(webdir + "php/" + file, "r", encoding="utf8") as inf:
            for line in inf:
    ...
    for folder in os.listdir(webdir + "vis"):
        if not folder.endswith(".html"):
            for file in os.listdir(webdir + "vis/" + folder):
                with open(
                    webdir + "vis/" + folder + "/" + file, "r", encoding="utf8"
                ) as inf:
                    for line in inf:
                        for php in params_php:
                            if php in line:
    ...
    def db(fn):
        ...
        con = sqlite3.connect(datadir + fn)
        cur = con.cursor()
        res = "\n" + "#" * len(fn) + "\n" + fn + "\n" + "#" * len(fn) + "\n"
    ...
    print(
        "\tDatabase Fingerprints for Consistency Checks in doc/data_consistency-"
        + curtime
        + ".txt"
    )
    with open("doc/data_consistency-" + curtime + ".txt", "a", encoding="utf8") as out:
        for file in os.listdir(datadir):
            if os.path.isdir(datadir + file):
                for file2 in os.listdir(datadir + file):
                    out.write(
                        file
                        + "/"
                        + file2
                        + "\t"
                        + str(os.path.getsize(datadir + file + "/" + file2))
                        + "\t"
                        + file2md5(datadir + file + "/" + file2)
                        + "\t"
                        + lastmodified(datadir + file + "/" + file2)
                        + "\n"
                    )
            else:
                out.write(
                    file
                    + "\t"
                    + str(os.path.getsize(datadir + file))
                    + "\t"
                    + file2md5(datadir + file)
                    + "\t"
                    + lastmodified(datadir + file)
                    + "\n"
                )

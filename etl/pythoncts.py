...
def cts_nslist():
    global ctsurl
    res = ""
    data = urlopenWithRetry("https://urncts.eu/namespaceresolver/index.php")
    for line in data:
...
def cts_nsurl(ns):
    global ctsurl
    try:
        data = urlopenWithRetry("https://urncts.eu/namespaceresolver/" + ns)
        for line in data:
        ...
        print(
            "ERROR: CTS Instance "
            + ns
            + " is not a registered CTS namespace. If you want to use an unregistered data set, you can set the endpoint URL manually in the file pythoncts.py"
        )
        exit()

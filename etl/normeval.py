...
incons = 0
with (
    open(datadir + "normmapping/_all.txt", "r", encoding="utf8") as bwin,
    open(datadir + "normmapping/_inconsistencies.txt", "w", encoding="utf8") as incout,
):
    for line in bwin:
        linearr = line.split("\t")
        if linearr[0] in bl:
...
with (
    ...
    open(datadir + "normmapping/_yay.txt", "w", encoding="utf8") as outyay,
    open(datadir + "normmapping/_nay.txt", "w", encoding="utf8") as outnay,
):
    for line in bwin:
        linearr = line.split("\t")
        if linearr[0] in bl:
...
with open(
    datadir + "normmapping/_progress.txt", "w", encoding="utf8"
) as outlemprogress:
...
with open(datadir + "normmapping/_stats.txt", "w", encoding="utf8") as out:
...
with open(datadir + "normmapping/_all.txt", "r", encoding="utf8") as inf:
    for line in inf:
        linearr = line.split("\t")
        norm = linearr[1]
        norm = norm[1:-1]
...
with open(datadir + "normmapping/_normuniqueness.txt", "w", encoding="utf8") as outf:
    for norm, value in sorted(
        normuniquenessbag.items(), key=lambda x: x[1], reverse=True
    ):
        outf.write(
            norm
            + "\t"
            + str(normuniquenessbag[norm])
            + "\t"
            + str(normunique[norm])
            + "\t"
            + str(normambiquebag[norm])
            + "\n"
        )
